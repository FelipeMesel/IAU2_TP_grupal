...
Para ello, comenzaremos por visualizar nuestros datos con  **gráficos de distribución** con regresión lineal y **gráficos de barras**.  
...
En este apartado, utilizaremos mapas de calor para ilustrar la variación de los precios de la oferta de viviendas en venta por barrio y por comuna, agrupando y resumiendo las observaciones trbajadas en el punto anterior. 
...
Ahora buscaremos encontrar la relación entre el mapa de precios de la ciudad y los equipamientos e infraestructuras urbanas.
...
establecimientos_educativos <- read_sf("data/establecimientos_educativos.geojson") %>% 
                               select(nombre_est, nivel=nivelmodal, dom_edific, comuna, barrio) %>% 
                               filter(comuna == "8" | comuna == "14" | comuna == "1" & nombre_est != "Centro de Formación Profesional - Cetic (Centro de Estudios Técnicos de la Industria de la Confección)")  #sacamos un establecimiento que identificamos tiene mal su geometría
...
icons_hosp <- awesomeIcons(icon = "hospital-o",
                      iconColor = "black",
                      library = "fa",
...
leaflet() %>%
  ...
  addAwesomeMarkers(data=hospitales,~lon, ~lat, popup = ~nombre, icon=icons_hosp) %>%
  addAwesomeMarkers(data=cesacs,~lon, ~lat, popup = ~nombre, icon=icons_cesac) %>% 
  addAwesomeMarkers(data=universidades,~long, ~lat, popup = ~universida, icon=icons_univ) 
```